...
//		let moveX = Motion.to(square, duration: 1, options: [.X(point.x)]).ease(Easing.inOutQuad)
//		let moveY = Motion.to(square, duration: 1, options: [.Y(point.y)]).ease(Easing.inOutQuad)
//		timeline.add([moveX, moveY], position: timeline.totalTime, align: .Start)
//		timeline.play()
		
		let move = Motion.to(square, duration: 1, options: [.Position(point.x, point.y)]).ease(Easing.inOutQuad)
		move.play()
	}
